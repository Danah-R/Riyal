...
String? validateProfileField(String field, String value) {
  ...
  if (text.isEmpty) return Strings.t('validation_required');
  if (field == 'Full name' &&
      (text.runes.length < 2 ||
          text.runes.length > 80 ||
          !RegExp(r'[a-zA-Z\u0621-\u064A]').hasMatch(text))) {
    return Strings.t('validation_name_length');
  }
  if (field == 'Email' &&
      !RegExp(r'^[^\s@]+@[^\s@]+\.[^\s@]+$').hasMatch(text)) {
    return Strings.t('validation_email');
  }
  if (field == 'Phone number') {
    final phone = normalizeProfilePhone(text);
    if (phone.startsWith('+966')) {
      if (!RegExp(r'^\+9665\d{8}$').hasMatch(phone)) {
        return Strings.t('validation_phone_local');
      }
    } else if (!RegExp(r'^\+[1-9]\d{7,14}$').hasMatch(phone)) {
      return Strings.t('validation_phone_country_code');
    }
  }
  return null;
}
