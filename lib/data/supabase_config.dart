...
/// Reads Supabase project config from the (gitignored) `.env` file and
...
class SupabaseConfig {
  ...
  static String get url {
    ...
    if (value == null || value.isEmpty) {
      throw StateError(
        'SUPABASE_URL is not set. Add it to .env (see .env.example).',
      );
    }
    return value;
  }
  ...
  static String get anonKey {
    ...
    if (value == null || value.isEmpty) {
      throw StateError(
        'SUPABASE_ANON_KEY is not set. Add it to .env (see .env.example).',
      );
    }
    return value;
  }
